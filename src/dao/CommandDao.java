...
public class CommandDao {
	...
	public int createCommande(Command commande) {
		...
		try (Connection con = DatabaseConnection.getConnection()) {
			...
			ex.printStackTrace();
			JOptionPane.showMessageDialog(null, "Erreur lors de la creation de la commande: " + ex.getMessage(),
	...
	public boolean updateStatut(int idCommande, String statut) {
		...
		try (Connection con = DatabaseConnection.getConnection()) {
			...
			JOptionPane.showMessageDialog(null,
					"Erreur lors de la mise à jour du statut de la commande: " + ex.getMessage(), "Erreur",
	...
	public boolean deleteCommande(int idCommande) {
		try (Connection con = DatabaseConnection.getConnection()) {
			...
			JOptionPane.showMessageDialog(null, "Erreur lors de la suppression de la commande: " + ex.getMessage(),
					"Erreur", JOptionPane.ERROR_MESSAGE);
		}
		return false;
	}
	...
	public List<Command> searchCommandesByFournisseurSociete(String societe) {
		...
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			while (rs.next()) {
				...
				Command cmd = new Command(tsDateCmd != null ? tsDateCmd.toLocalDateTime().toLocalDate() : null,
						tsDateRec != null ? tsDateRec.toLocalDateTime().toLocalDate() : null, statut, prix, supplier);
				cmd.setIdCommande(idCommande);
				list.add(cmd);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			JOptionPane.showMessageDialog(null, "Erreur en recherchant commandes par fournisseur: " + ex.getMessage(),
					"Erreur", JOptionPane.ERROR_MESSAGE);
		}
		return list;
	}
	...
	public List<Command> searchCommandesByMedicamentNom(String nom) {
		...
		String sql = "SELECT DISTINCT c.id_commande, c.date_commande, c.date_reception, c.statut, c.prix, "
				+ "f.id_fournisseur, f.nom, f.prenom, f.societe, f.email, f.telephone, f.adresse, f.description "
				+ "FROM commande c " + "JOIN commandeproduit cp ON cp.id_commande = c.id_commande "
				+ "JOIN medicament m ON cp.code_barre = m.code_barre "
				+ "LEFT JOIN fournisseur f ON c.id_fournisseur = f.id_fournisseur " + "WHERE m.nom LIKE ?";
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			ex.printStackTrace();
			JOptionPane.showMessageDialog(null, "Erreur en recherchant commandes par medicament: " + ex.getMessage(),
					"Erreur", JOptionPane.ERROR_MESSAGE);
		}
		return list;
	}
	...
	public Command getCommandeById(int idCommande) {
		...
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			if (rs.next()) {
				...
				cmd = new Command(tsDateCmd != null ? tsDateCmd.toLocalDateTime().toLocalDate() : null,
						tsDateRec != null ? tsDateRec.toLocalDateTime().toLocalDate() : null, statut, prix, supplier);
			...
			ex.printStackTrace();
			JOptionPane.showMessageDialog(null, "Erreur en chargeant la commande: " + ex.getMessage(), "Erreur",
					JOptionPane.ERROR_MESSAGE);
		}
		return cmd;
	}
	...
	public List<Command> getAllCommands(boolean pending) {
		...
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			ex.printStackTrace();
			JOptionPane.showMessageDialog(null, "Erreur en chargeant commandes: " + ex.getMessage(), "Erreur",
					JOptionPane.ERROR_MESSAGE);
		}
		return list;
	}
	...
	public List<Command> getReceivedCommandsReceivedBySupplier(Supplier supplier) {
		List<Command> list = new ArrayList<>();
		String sql = "SELECT c.id_commande, c.date_commande, c.date_reception, c.statut, c.prix " + "FROM commande c "
		...
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			ex.printStackTrace();
			JOptionPane.showMessageDialog(null,
					"Erreur en chargeant commandes reçues par fournisseur: " + ex.getMessage(), "Erreur",
					JOptionPane.ERROR_MESSAGE);
		}
		return list;
	}
