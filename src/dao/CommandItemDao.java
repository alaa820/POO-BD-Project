...
public class CommandItemDao {
	...
	public List<CommandItem> searchByFournisseurAndMedicament(String supplierName, String medicineName) {
		List<CommandItem> list = new ArrayList<>();
		String sql = "SELECT cp.* FROM commandeproduit cp " + "JOIN commande c ON cp.id_commande = c.id_commande "
				+ "JOIN fournisseur f ON c.id_fournisseur = f.id_fournisseur "
				+ "JOIN medicament m ON cp.code_barre = m.code_barre " + "WHERE f.societe LIKE ? AND m.nom LIKE ?";
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement pst = con.prepareStatement(sql)) {
	...
	public List<CommandItem> searchByFournisseur(String supplierName) {
		List<CommandItem> list = new ArrayList<>();
		String sql = "SELECT cp.* FROM commandeproduit cp " + "JOIN commande c ON cp.id_commande = c.id_commande "
				+ "JOIN fournisseur f ON c.id_fournisseur = f.id_fournisseur " + "WHERE f.societe LIKE ?";
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement pst = con.prepareStatement(sql)) {
			pst.setString(1, "%" + supplierName + "%");
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
	...
	public List<CommandItem> searchByMedicine(String medicineName) {
		List<CommandItem> list = new ArrayList<>();
		String sql = "SELECT cp.* FROM commandeproduit cp " + "JOIN medicament m ON cp.code_barre = m.code_barre "
				+ "WHERE m.nom LIKE ?";
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement pst = con.prepareStatement(sql)) {
	...
	public List<CommandItem> getProduitsByCommande(int idCommande) {
		...
		try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			ex.printStackTrace();
			JOptionPane.showMessageDialog(null, "Erreur en chargeant produits de la commande: " + ex.getMessage(),
	...
	public List<CommandItem> getAllCommandItemsReceived() {
		List<CommandItem> list = new ArrayList<>();
		try {
			String sql = "SELECT * FROM commandeproduit cp " + "JOIN commande c ON cp.id_commande = c.id_commande "
